...
namespace MahdiGhiasi.AppListManager
{
    ...
    public class LoadAppData
    {
        ...
        public async Task LoadApps()
        {
            ...
            {
                //System.Diagnostics.Debug.WriteLine(current_pos);
                ...
                current_pos++;
                OnLoadingProgress(new LoadingEventArgs(current_pos, packageCount));
            }
            ...
            Windows.ApplicationModel.Package item, StorageFolder saveLogoLocation)
        {
            ...
            try
            {
                ...
                // Summary:
                // Lists the packaged apps on the device. Only packaged
                // applications registered for the current user are returned.
                //
                // Return:
                // A list of packaged applications along with their display name, description, and logo.
                IReadOnlyList<Windows.ApplicationModel.Core.AppListEntry> x 
                                                                  = await item.GetAppListEntriesAsync();
        ...
        public static async Task SaveAppList()
        {
            ...
            StorageFolder localCacheFolder = ApplicationData.Current.LocalCacheFolder;
            StorageFile cacheFile = await localCacheFolder.CreateFileAsync("applistcache.txt", 

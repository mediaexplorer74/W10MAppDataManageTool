    ...
    class UpdateChecker
    {
        ...
        public static async void CheckForUpdates()
        {
            try
            {
                ...
                string text = await MakeWebRequest("http://www.ghiasi.net/W10MAppDataManageTool/latestversion.txt?dtcache=" + DateTime.Now.ToString("yyyyMMddHHmmssfff"));
                ...
                if (VersionToNumber(latestVersion) > VersionToNumber(currentVersion))
                {
                    MessageDialog md = new MessageDialog("App Data Manage Tool v" + text + " is available now.\r\n\r\n" +
                        "You can download it from:\r\nhttp://bit.ly/W10MAppDataManageTool", "A newer version of App Data Manage Tool is available!");
                    await md.ShowAsync();
                }
            }
            catch
